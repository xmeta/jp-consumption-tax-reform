...
for r in rows:
    opening = int(r["opening_incremental_debt_yen"])
    redemptions = int(r["redemptions_yen"])
    net_new = int(r["net_new_issuance_yen"])
    gross_new = int(r["gross_new_issuance_yen"])
    closing = int(r["closing_incremental_debt_yen"])
...
for sid in ("current_8_10", "full_abolition_income_tax", "full_abolition_asset_tax"):
    assert all(int(r["closing_incremental_debt_yen"]) == 0 for r in rows if r["scenario_id"] == sid)
    assert all(int(r["incremental_interest_expense_yen"]) == 0 for r in rows if r["scenario_id"] == sid)
...
for aid in {r["assumption_set_id"] for r in rows}:
    for year in range(1, 11):
        full = int(by_key[("full_abolition_jgb", aid, str(year))]["closing_incremental_debt_yen"])
        mixed = int(by_key[("full_abolition_mixed", aid, str(year))]["closing_incremental_debt_yen"])
        assert mixed * 2 == full

low = by_key[("full_abolition_jgb", "r01_g04", "10")]
high = by_key[("full_abolition_jgb", "r04_g00", "10")]
assert float(high["incremental_debt_gdp_ratio"]) > float(low["incremental_debt_gdp_ratio"])
assert int(high["cumulative_incremental_interest_yen"]) > int(low["cumulative_incremental_interest_yen"])
assert any(int(r["redemptions_yen"]) > 0 for r in rows if r["scenario_id"] == "full_abolition_jgb" and r["year"] != "1")
