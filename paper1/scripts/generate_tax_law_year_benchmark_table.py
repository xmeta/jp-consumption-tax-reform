...
def pct(x): return f"{100.0 * float(x):.4f}%"
def pp(x): return f"{100.0 * float(x):.4f} pp"
def interval(r, lo, hi): return f"{100.0*float(r[lo]):.4f}–{100.0*float(r[hi]):.4f}%"

def build():
    rows=read()
    if {int(r["tax_law_year"]) for r in rows} != {2024,2026}: raise RuntimeError("tax-law benchmark year set changed")
    out=[]
    for r in sorted(rows, key=lambda x:int(x["tax_law_year"])):
        out.append({
            "Tax law applied to 2024 aggregates": r["tax_law_year"],
            ...
            "v5 taxable-MTR envelope at Delta=0": interval(r,"v5_delta_0_taxable_mtr_min","v5_delta_0_taxable_mtr_max"),
        })
    return out
...
def main():
    ...
    if args.check:
        ...
        print("paper1 tax-law-year benchmark table: current")
    else:
        OUTPUT.write_text(expected,encoding="utf-8"); print(f"wrote {OUTPUT.relative_to(ROOT)}")
