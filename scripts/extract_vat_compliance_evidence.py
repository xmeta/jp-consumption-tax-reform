...
def build():
    ...
    # Post-2023 invoice-era supplier-network evidence from JCCI 2024/2025.
    # Preserve realized status, stated future intentions, registration pressure and substitution frictions.
    for jr in jcci_network_rows:
        rows.append(evidence(
          f"jcci_network_{jr['survey_year']}_{jr['metric_id']}", jr["value"], jr["unit"],
          jr["evidence_type"], jr["response_scope"], jr["denominator_n"],
    ...
    ident=[
      ...
      {"quantity":"invoice_era_future_price_or_supplier_review","status":"OBSERVED_STATED_FUTURE_INTENTION","point_identified":"42.3_PERCENT_AMONG_N502","model_use":"INTENTION_NOT_REALIZED_NETWORK_EFFECT","note":"JCCI 2025 reports that 42.3% of respondents currently purchasing from exempt businesses plan to review transaction prices or suppliers: 30.5% price review, 9.8% mostly stop purchases, 2.0% stop all. This is stated intention, not realized network reallocation."},
